...
export default class VideoSheetStatus extends React.Component {
    constructor(props) {
        super(props)
        this.state = {
            loader: false,
    ...
    getSheetList = async (student_id, subject_id) => {
        ...
        let postData = {
            ...
            action: 'getstudent_submittedvideos',
            subject_id
        }
    ...
    goToReportCard = (id, status) => {
        if (status == "Reviewed") {
            this.props.navigation.navigate('VideoSheetReportDetails', {
                sheet_id: id
            })
        } else {
            this.setState({ showAlert: true, error_message: 'Video not yet reviewed' })
        }
    }
    goToPendingVideoSheets = () => {
        this.props.navigation.navigate('PendingVideoSheets', {
    ...
    render() {
        ...
        return (
            <View style={styles.container}>
                ...
                />
                <Header title="Submitted" title1="Video Status" backScreen="SheetVideoMenu" headerImage="confirmSubmit" navigation={this.props.navigation} />
                        ...
                        <View style={styles.aboutContainer}>
                            <View style={styles.tabContianer}>
                                <View style={styles.tab1}>
                                    <Text style={styles.tabText}>SUBMITTED VIDEOS</Text>
                                </View>
                                <TouchableWithoutFeedback onPress={this.goToPendingVideoSheets}>
                                    <View style={styles.tab}>
                                        <Text style={styles.tabText1}>PENDING VIDEOS</Text>
                                    </View>
                                </TouchableWithoutFeedback>
                            </View>
                        </View>
                                    ...
                                    <TouchableWithoutFeedback onPress={() => this.goToReportCard(item.id, item.status)} key={item.id}>
                                        <Card containerStyle={styles.card}>
                                            <View style={styles.row}>
                                                ...
                                                <View>
                                                    <Text style={item.status == "Reviewed" ? styles.status1 : styles.status}>{item.status}</Text>
                                                </View>
                                            </View>
                                        </Card>
                                    </TouchableWithoutFeedback>
