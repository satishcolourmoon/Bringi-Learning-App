...
export default class LearnSelectSubject extends React.Component{
    ...
    goToMedium = (id)  => {
        this.props.navigation.navigate('SelectMedium',{
            board_id : id,
    ...
    render(){
        ...
        return(
            <View style={styles.container}>
                <StatusBar hidden />
                <Header title="Learn More" title1="" backScreen="LearnSelectSubject"  headerImage="learn_more" navigation={this.props.navigation} />
                    <View style={{flex:0.9}}>
                       <ScrollView>
                            <View style={styles.selectContainer}>
                                <Text style={styles.heading1}>Select Board</Text>
                            </View>
                            <View style={styles.aboutContainer}>
                                {
                                    this.state.data.map((item,index)=>{
                                        return(
                                            <TouchableWithoutFeedback onPress={()=>this.goToMedium(item.id)} key={index}>
