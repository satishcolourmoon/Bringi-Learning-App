...
export default class LearnSelectSubject extends React.Component {
    ...
    goToSelectBoard = (subject_id) => {
        this.props.navigation.navigate('SelectBoard', {
    ...
    render() {
        ...
        return (
            <View style={styles.container}>
                <StatusBar hidden />
                <Header title="Learn More" title1="" backScreen="Home" headerImage="learn_more" navigation={this.props.navigation} />
                <View style={{ flex: 0.9 }}>
                    <ScrollView>
                        <View style={styles.selectContainer}>
                            <Text style={styles.heading1}>Select Subject</Text>
                        </View>
                        <View style={styles.aboutContainer}>
                            {
                                this.state.data.map((item, index) => {
                                    return (
                                        <TouchableWithoutFeedback onPress={() => this.goToSelectBoard(item.id)} key={index}>
