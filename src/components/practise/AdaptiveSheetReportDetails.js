...
export default class AdaptiveSheetReportDetails extends React.Component {
    constructor(props) {
        super(props)
        this.state = {
            loader: false,
    ...
    componentDidMount() {
        ...
        Asyncstorage.getItem('student_id').then((student_id) => {
            this.setState({ sheet_id: sheet_id, loader: true, student_id }, () => {
                this.getReportCardDetails(sheet_id, student_id)
            })
        })
    }
    goBack = () => {
    ...
    getReportCardDetails = (sheet_id, student_id) => {
        fetch(baseUrl + 'user', {
            method: 'POST',
            headers: {
