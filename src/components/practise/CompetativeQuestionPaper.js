...
export default class CompetativeQuestionPaper extends React.Component {
    constructor(props) {
        super(props)
        this.state = {
            loader: true,
    ...
    componentDidMount() {
        ...
        Asyncstorage.getItem('student_id').then((student_id) => {
            this.setState({ student_id, test_id }, () => {
                this.getQuestions(student_id, test_id);
            })
    ...
    getQuestions = (student_id, test_id) => {
        fetch(baseUrl + 'user', {
            method: 'POST',
            headers: {
            ...
            body: qs.stringify({
                action: 'compitative_tests_load_Questions',
                test_id: test_id,
            ...
            .then((json) => {
                if (!json.status) {
                    ...
                    if (json.test_list.noofquestion > 0) {
                        Asyncstorage.getItem('user_id').then((user_id) => {
                            this.setState({
                                loader: false,
                                data: json.test_list.questions,
                                noOfQns: json.test_list.noofquestion,
                                question_name: json.test_list.questions[0].question,
                                options: json.test_list.questions[0].options,
                                question_id: json.test_list.questions[0].id,
                                time: json.test_list.total_time,
                                index: 0,
                                parent_id: user_id,
                                student_id: student_id,
                                userAnswer: json.test_list.questions[0].user_answer,
                                image: json.test_list.questions[0].image,
                                question_type: json.test_list.questions[0].question_type,
                            }, () => {
                                this.startExamTimer(test_id, student_id)
                            });
                        })
                    } else {
    ...
    startExamTimer = async (test_id, user_id) => {
        ...
        fetch(baseUrl + 'user', {
            ...
            body: qs.stringify({
                action: 'competative_practice_exam_start_timer',
                user_id,
                test_id
            })
        }).then((response) => response.json())
    ...
    selectNextQuestion = (index) => {
        ...
        fetch(baseUrl + 'user', {
            ...
            body: qs.stringify({
                action: 'save_compitative_tests_question',
                            ...
                            .then((json) => {
                                if (!json.status) {
                                    ...
                                    this.setState({
                                        loader: false,
                                        data: json.test_list.questions,
                                        userAnswer: this.state.test_list[index + 1].user_answer
                                    });
                                }
                            }).catch((error) => {
                                this.setState({ loader: false });
    ...
    submitExam = (index) => {
        ...
        fetch(baseUrl + 'user', {
            ...
            body: qs.stringify({
                action: 'compitative_tests_finished',
                test_id: this.state.test_id,
                parent_id: this.state.parent_id,
                user_id: this.state.student_id,
                question_id: this.state.question_id,
